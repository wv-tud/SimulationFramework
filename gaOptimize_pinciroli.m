...
simt    = toc(pT);
options = optimoptions('ga','PopulationSize',150, 'Display','iter', 'PlotFcn',{@gaplotbestf_scale @gaplotbestindiv @gaplotgenealogy},'UseParallel',1);
LB      = zeros(1,2);
UB      = 1 * ones(1,2);
fprintf(strcat(['Simulation took ' num2str(simt) 's - pinciroliPerformance: (cost: ' num2str(cost) ')\n\n']));
[x,fval,exitflag,output,population,scores] = ga(@sim_calc_cost_pinciroli, 2,[],[],[],[],LB,UB,[],options);
